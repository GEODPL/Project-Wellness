...
def exercise_followup_reply(user_text: str) -> str:
    """
    Ειδική απάντηση όταν η άσκηση ζητά «διάλεξε μια λέξη».
    Παίρνουμε ό,τι έγραψε ο χρήστης και το σχολιάζουμε ήπια.
    """
    raw = user_text.strip()
    if not raw:
        return (
            "Δεν χρειάζεται να το σκεφτείς πολύ – ακόμη κι αν δεν σου βγει «η τέλεια» λέξη, "
            "οτιδήποτε σου έρχεται πρώτο στο μυαλό είναι ήδη χρήσιμο για να δούμε λίγο "
            "πώς σε αγγίζει αυτή η άσκηση."
        )

    word = raw.split()[0].lower()

    # Πολύ πρόχειρα mini-lexicons για συναίσθημα της λέξης
    anger_words = ["θυμ", "νευρ", "οργ", "εκνευρ", "αγανάκτ"]
    sad_words = ["λυπ", "στεναχ", "θλιβ", "μοναξ", "απογοητ", "κενό", "κενο"]
    tired_words = ["κουρα", "εξαντλ", "burnout", "διαλυμ", "τελειωμ"]
    hope_words = ["ελπίδ", "ελπι", "χαρ", "χαμόγ", "ηρεμ", "ηρεμ", "ανάσα", "ανασα"]

    def matches(lst):
        return any(stem in word for stem in lst)

    if matches(anger_words):
        tone = (
            "Η λέξη που διάλεξες έχει αρκετή ένταση μέσα της. "
            "Μου δίνει την αίσθηση ότι κουβαλάς θυμό ή εκνευρισμό, και αυτό είναι απολύτως ανθρώπινο. "
            "Μπορούμε στα επόμενα check-ins να δούμε σιγά σιγά από πού έρχεται όλη αυτή η ένταση "
            "και πώς μπορείς να της δίνεις λίγο χώρο χωρίς να σε πνίγει."
        )
    elif matches(sad_words):
        tone = (
            "Η λέξη που σημείωσες μοιάζει να κουβαλάει βάρος και ίσως λίγη μοναξιά. "
            "Δεν χρειάζεται να την αναλύσουμε τώρα – αρκεί που την παρατήρησες και την κατέγραψες. "
            "Αν το θέλεις, μπορούμε σιγά σιγά να δούμε τι χρειάζεσαι όταν αυτό το αίσθημα "
            "γίνεται πιο δυνατό μέσα στην ημέρα."
        )
    elif matches(tired_words):
        tone = (
            "Από τη λέξη που διάλεξες, μου έρχεται στο μυαλό κούραση ή εξάντληση. "
            "Ίσως να έχεις τραβήξει πολλά για καιρό. "
            "Μπορούμε στα επόμενα βήματα να σκεφτούμε 1–2 πολύ μικρές κινήσεις ξεκούρασης, "
            "όχι τέλειες λύσεις, αλλά κάτι που να σε στηρίζει λίγο μέσα στην ημέρα."
        )
    elif matches(hope_words):
        tone = (
            "Μου αρέσει η λέξη που διάλεξες – αφήνει χώρο για λίγη ελπίδα ή ανακούφιση, "
            "ακόμη κι αν τα πράγματα δεν είναι εύκολα. "
            "Μπορούμε να κρατήσουμε αυτή τη λέξη ως ένα μικρό «σήμα» για να θυμάσαι "
            "τι σε βοηθά έστω και λίγο όταν τα πράγματα ζορίζουν."
        )
    else:
        tone = (
            "Κρατάω τη λέξη «" + raw + "» που σημείωσες. "
            "Δεν έχει σημασία αν μοιάζει «σωστή» ή «λάθος» – το ότι την επέλεξες "
            "λέει ήδη κάτι για το πώς βιώνεις αυτή τη στιγμή. "
            "Αν θες, μπορούμε σταδιακά να δούμε πότε αυτή η λέξη ταιριάζει περισσότερο "
            "στην ημέρα σου και πότε λιγότερο."
        )

    return (
        "Σ’ ευχαριστώ που μοιράστηκες αυτή τη λέξη.\n\n"
        + tone
    )
